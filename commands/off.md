...
The command persists a single boolean per CC session to `~/.claude/audio-recap/state/<session-id>.json`, drops this session's presence heartbeat under `~/.claude/audio-recap/active/` so concurrent sessions stop announcing their names, and prints a one-line confirmation. Do not interpret, rephrase, or add commentary to its output — quote it to the user directly.
